...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    cls_num = 10
    def __init__(self, root, quantity_ratio,imb_type='exp', imb_factor=0.01,weighted_sampler='default', rand_number=0, train=True,
        ...
        self.class_dict=self._get_class_dict()
        print("aaa is",self.instance_p)
        print("bbb is",self.class_p)

    def update(self, epoch):
        self.epoch = epoch
        print('epoch in dataset', self.epoch)
        if self.weighted_sampler == "pps":
          if self.epoch<self.epoch_min:
            self.pp=self.instance_p
          elif self.epoch>=self.epoch_max:
            self.pp=self.class_p
          else:
            self.pp= (((self.epoch-self.epoch_min)/(self.epoch_max-self.epoch_min))**self.alpha) * self.class_p \
                     + (1-(((self.epoch-self.epoch_min)/(self.epoch_max-self.epoch_min))**self.alpha))*self.instance_p
...
class CIFAR10_LT(object):
    ...
    def __init__(self,root='./data/imbalance_cifar10',quantity_ratio=1.0, imb_type='exp',
                 imb_factor=0.01,weighted_sampler='default', batch_size=128, num_works=40):
        ...
        train_dataset = IMBALANCECIFAR10(root=root, quantity_ratio=quantity_ratio,imb_type=imb_type, imb_factor=imb_factor, weighted_sampler=weighted_sampler,rand_number=0, train=True,
