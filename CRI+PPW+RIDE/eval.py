...
def main(config):
    ...
    if hasattr(model, "confidence_model") and model.confidence_model:
        ...
        cumulative_sample_num_experts_each_shot = [
            torch.zeros((model.backbone.num_experts,), dtype=torch.float, device=device) for _ in range(3)]
        num_samples = 0
    ...
    if confidence_model:
        ...
        print({"many_hp_num": (cumulative_sample_num_experts_each_shot[0] / cumulative_sample_num_experts_each_shot[
            0].sum()).cpu().tolist(),
               "medium_hp_num": (cumulative_sample_num_experts_each_shot[1] / cumulative_sample_num_experts_each_shot[
                   1].sum()).cpu().tolist(),
               "few_hp_num": (cumulative_sample_num_experts_each_shot[2] / cumulative_sample_num_experts_each_shot[
                   2].sum()).cpu().tolist()})
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default='./config/cifar10.json', type=str,
