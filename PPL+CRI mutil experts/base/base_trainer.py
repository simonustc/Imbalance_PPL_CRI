...
class BaseTrainer:
    ...
    def __init__(self, model, k, cls_num_list, criterion, metric_ftns, optimizer, config):
        self.config = config
        ...
        self.real_model = self.model.to(self.device)
        ...
        self.checkpoint_dir = os.path.join(config.save_dir, str(k))
    ...
    def train(self, k):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if epoch > 92:
                ...
                if log['val_accuracy'] >= best_result:
                    best_result = log['val_accuracy']
                    best_epoch = log['epoch']
                    self._save_checkpoint(best_epoch, save_test_best=True)
                # print('best_result:    ',best_result*100)
                # print('best_epoch:    ', best_epoch)
                self.logger.info('best:{}   epoch:{}'.format(best_result * 100, best_epoch))
        ...
        best_dir = os.path.join(self.checkpoint_dir, str(best_result * 100))
        if not os.path.exists(best_dir):
            os.makedirs(best_dir)

    def _prepare_device(self, n_gpu_use, gpus):
        """
        setup GPU device if available, move model into configured device
        """
    ...
    def _save_checkpoint(self, epoch, save_best=False, best_only=False, save_test_best=False):
        """
        Saving checkpoints
        ...
        if save_test_best:
            test_best_path = str(self.checkpoint_dir + 'model_test_best.pth')
            torch.save(state, test_best_path)
            # self.logger.info("Saving test best: {} ...".format(test_best_path))
            print("Saving test best: {} ...".format(test_best_path))
