...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, k, cls_num_list, criterion, metric_ftns, optimizer, config, data_loader,
                 valid_data_loader=None, lr_scheduler=None, len_epoch=None):
        super().__init__(model, k, cls_num_list, criterion, metric_ftns, optimizer, config)
        self.config = config
        ...
        if self.distill:
            ...
            if len(self.device_ids) > 1:
                ...
                load_state_dict(self.teacher_model, {"module." + k: v for k, v in teacher_state_dict.items()},
                                no_ignore=True)
            else:
    ...
    def _train_epoch(self, epoch):
        ...
        if hasattr(self.criterion, "_hook_before_epoch"):
            self.criterion._hook_before_epoch(epoch, self.device)
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            if self.config["trainer"]["scheduler"] == 'default':
            ...
            elif self.config["trainer"]["scheduler"] == 'mixup':
                ...
                with autocast():
                    if self.real_model.requires_target:
                        ...
                        if self.distill:
                            ...
                            # loss = self.criterion(output_logits=output, target=target)
                            loss = lam * self.criterion(output_logits=output, target=targets_a) + (
                                        1 - lam) * self.criterion(output_logits=output, target=targets_b)

            elif self.config["trainer"]["scheduler"] == 'remix':
                cls_num_list1 = torch.FloatTensor(self.cls_num_list)

                l = np.random.beta(0.5, 0.5)
                idx = torch.randperm(data.size(0)).to(self.device)
                image_a, image_b = data, data[idx]
                label_a, label_b = target, target[idx]
                mixed_image = l * image_a + (1 - l) * image_b
                data = mixed_image.to(self.device)
                ...
                with autocast():
                    if self.real_model.requires_target:
                        ...
                        n_i = cls_num_list1[label_a].float().to(self.device)
                        n_j = cls_num_list1[label_b].float().to(self.device)
                        ...
                        if self.distill:
                            ...
                            # loss = self.criterion(output_logits=output, target=target)
                            loss = l_list * self.criterion(output_logits=output, target=label_a) + (
                                        1 - l_list) * self.criterion(output_logits=output, target=label_b)
                            loss = loss.mean()

            elif self.config["trainer"]["scheduler"] == 'ppmix':
                cls_num_list1 = torch.FloatTensor(self.cls_num_list)
                ...
                with autocast():
                    if self.real_model.requires_target:
                        ...
                        n_i = cls_num_list1[label_a].float().to(self.device)
                        n_j = cls_num_list1[label_b].float().to(self.device)
                        ppmix_epoch_min = self.config["trainer"]["epoch_min"]
                        ppmix_epoch_max = self.config["trainer"]["epoch_max"]

                        if l < 0.5:
                            if epoch < ppmix_epoch_min:
                                l_list[n_i / n_j >= 3] = l
                            if epoch >= ppmix_epoch_min and epoch <= ppmix_epoch_max:
                                l_list[n_i / n_j >= 3] = (l / (
                                        ppmix_epoch_min - ppmix_epoch_max)) * epoch + (ppmix_epoch_max * l) / (
                                                                     ppmix_epoch_max - ppmix_epoch_min)
                            if epoch > ppmix_epoch_max:
                                l_list[n_i / n_j >= 3] = 0
                        if 1 - l < 0.5:
                            if epoch < ppmix_epoch_min:
                                l_list[(n_i * 3) / n_j <= 1] = l
                            if epoch >= ppmix_epoch_min and epoch <= ppmix_epoch_max:
                                l_list[(n_i * 3) / n_j <= 1] = ((1 - l) / (
                                        ppmix_epoch_max - ppmix_epoch_min)) * epoch + (
                                                                           (l * ppmix_epoch_max - ppmix_epoch_min) / (
                                                                               ppmix_epoch_max - ppmix_epoch_min))
                            if epoch > ppmix_epoch_max:
                                l_list[(n_i * 3) / n_j <= 1] = 1
            ...
            if batch_idx % self.log_step == 0:
                ...
                '''
                print('Train Epoch: {} {} Loss: {:.6f} max group LR: {:.4f} min group LR: {:.4f}'.format(
                    epoch,
                    self._progress(batch_idx),
                    loss.item(),
                    max([param_group['lr'] for param_group in self.optimizer.param_groups]),
        ...
        if self.do_validation and epoch > 92:
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            if confidence_model:
                print("Samples with num_experts:",
                      *[('%.2f' % item) for item in (cumulative_sample_num_experts * 100 / num_samples).tolist()])
