...
def main(config):
    ...
    for k in range(200):
        ...
        if hasattr(loss_class, "require_num_experts") and loss_class.require_num_experts:
            criterion = config.init_obj('loss', module_loss, cls_num_list=data_loader.cls_num_list,scheduler=config["loss"]["scheduler"],num_experts=config["arch"]["args"]["num_experts"])
        else:
            criterion = config.init_obj('loss', module_loss, cls_num_list=data_loader.cls_num_list,scheduler=config["loss"]["scheduler"])
        ...
        trainer = Trainer(model, k,cls_num_list,criterion, metrics, optimizer,
                          config=config,
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default='./config/cifar100.json', type=str,
